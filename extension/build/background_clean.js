    ...
    MyBlacklist.prototype.isBlacklisted = function(string, type) {
      ...
      if (lookupDir.length > 0) {
        if (type === "url") {
          ...
          if (string.search(r) >= 0) return true;
        }
        if (type === "keyword") {
    ...
    */
    ...
    WipeMode.prototype.purgeBadUrl = function(url) {
      ...
      */
      ...
      if (url.indexOf("www") >= 0) {
        ...
        return console.log("purged " + (httpsUrl.replace("https://www.", "https://")));
      } else {
        chrome.history.deleteUrl({
        ...
        return console.log("purged " + (httpsUrl.replace("https://", "https://www.")));
      }
    };
    ...
    WipeMode.prototype.wipeHistory = function(startTime, doFullClean) {
      var endTime, maxResults, site, visitStack, _i, _len, _ref,
        _this = this;
      if (!startTime) startTime = new Date(2000, 0, 1).getTime();
      ...
      chrome.history.search({
        ...
        var deleteCount, hItem, nastyRedirect, visitCount, _j, _k, _len2, _len3;
        deleteCount = 0;
        visitCount = 0;
        for (_j = 0, _len2 = historyItems.length; _j < _len2; _j++) {
          ...
          if (myBlacklist.isBlacklisted(hItem.url, "url") || myBlacklist.isBlacklisted(hItem.title, "keyword")) {
            chrome.history.getVisits({
              ...
              var visitItem, _k, _len3, _results;
